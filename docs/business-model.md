# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Sweden

## Classification

- Repository: `cloud-itonami-iso3166-swe`
- ISO 3166: `SWE` (Sweden)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Swedish
  public contract
...
- registration walkthrough for TendSign (Visma TendSign), one of the
  e-procurement platforms most Swedish public agencies and municipalities
  publish tenders and accept electronic bids through
- business/tax registration checklist: an entry with Bolagsverket (the
  Swedish Companies Registration Office) and F-tax (F-skatt) / VAT
  registration with Skatteverket (the Swedish Tax Agency)
- local-content / preferential-procurement navigation: EU-wide open
  ...
  member state), but Swedish-language submission and a Nordic/EU address
  for service are typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Swedish-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Sweden).
