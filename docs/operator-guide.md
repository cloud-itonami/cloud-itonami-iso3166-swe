...
2. Register the client's intake: business type, target public function,
   prior filing history in Sweden if any.
3. Run the advisor in read-only mode against TendSign (Visma TendSign)
   tender notices.
4. Compare the checklist against the client's current documentation
   (Bolagsverket registration, F-skatt/VAT status with Skatteverket).
...
- named referral relationship with Swedish-licensed counsel or a
...
a working referral relationship with Swedish-licensed counsel or a
registered agent for whatever licensed representation the law of
Sweden requires for actual public-procurement filings.
